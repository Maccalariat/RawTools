...
FileMetaData::FileMetaData(FileBuffer& fileBuffer) :
	...
	tiff_ifd.clear();
	std::cout << "raw image in ifd offset" << raw_ifd.file_offset << " " << std::endl;
	}
...
void FileMetaData::processDE(const size_t offset, tiff_ifd_s& ifd) {
	...
	switch (tag) {
			...
			case 37:
			case 38:
				// cam_mul[tag - 36] = get2(); -- this does not make sense cam_mul is [4] long
				break;
			case 0X100:
				ifd.width = tag_value_offset;
				break;
			...
			case 274:
				...
				/* case 280:				// Panasonic RW2 offset
					...
					load_flags = 0x2008;
					*/    // note runs into next case 273 (shit code)
			...
			case 11:
				...
				if (!software.compare(0, 5, "Adobe") ||
					!software.compare(0, 5, "dcraw") ||
					!software.compare(0, 5, "UFRaw") ||
