...
FileMetaData::FileMetaData(FileBuffer &fileBuffer) : _fileBuffer(fileBuffer) {
    ...
    bigEndian = _fileBuffer.bigEndian;
    ...
    for (auto ifd : tiff_ifd) {
        if (ifd.ifd_type == 0) {
            raw_ifd = ifd;
            if (raw_ifd.Orientation == 0) {
                raw_ifd.Orientation = orientation;
            }
            break;
        }
    }
...
void FileMetaData::parseFile() {
    ...
    uint32_t next_IFD = static_cast<uint32_t>(first_offset);
    do {
...
void FileMetaData::processDE(const size_t offset, tiff_ifd_s &ifd) {
    ...
    switch (tag) {
        ...
        case 5:
            ifd.ImageWidth = tag_value_offset;
            break;
        case 6:
            ifd.ImageHeight = tag_value_offset;
            break;
        case 9: // ???
            /*
        ...
        */
        ...
        case 61443: {
            ifd.BitsPerSample = tag_value_offset;
            break;
        }
        case 0x103: // compression
            ifd.Compression = tag_value_offset;
            break;
        case 0X10E: {
        ...
        case 0x112: {               // orientation
            if (tag_value_offset > 0) {
                // strange omission of orientation in sony raw IFD. use whatever is there
                orientation = tag_value_offset;
            }
            ifd.Orientation = tag_value_offset;
            break;
        };
        ...
        case 277: // samples per pixel
            ifd.SamplesPerPixel = tag_value_offset;
            break;
        ...
        case 324: { // TileOffsets
            ...
            if (tag_count == 1) { // the image is in one tile (ie: most images)
