...
TiffFile::TiffFile(const std::string &fileName, const FileMetaData &metadata)
    : metaData(metadata), tagVector(4, 0) {
...
void TiffFile::write_file() {
  ...
  outputFile->write(reinterpret_cast<const char *>(&magic_number),
                    sizeof(magic_number));
  outputFile->write(reinterpret_cast<const char *>(&first_offset),
                    sizeof(first_offset));
  outputFile->write(reinterpret_cast<const char *>(&de_count),
                    sizeof(de_count));
  ...
  writeTag(PhotometricInterpretationTag, SHORT, ONE,
           ONE); // TODO figure this out - forced to BlackIsZero
  writeTag(StripOffsetsTag, SHORT, ONE,
           sizeof(magic_number) + sizeof(first_offset) + sizeof(de_count) +
               120);
  ...
  outputFile->write(reinterpret_cast<const char *>(&metaData.Green),
                    sizeof(metaData.Green));
